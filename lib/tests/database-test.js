(function() {
  ...
  vows.describe('Database Operations').addBatch({
    'An instance of a Database object': {
      ...
      'should return an error when there is an error on the call': function(db) {
        var mockResponse;
        mockResponse = {
          statusCode: 500,
      ...
      'should handle a non-JSON response when receiving an error on saveDocument': function(db) {
        var body, status;
      ...
      'should handle a non-JSON response when receiving an error on getDocumentCount': function(db) {
        var body, status;
        body = "<html><title>Permission Denied</title><body>You shall not pass!</body></html>";
        status = 401;
        helpers.mockApiCalls(db, status, {
